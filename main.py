...
def manage_tournament(controller: TournamentController): # Fonction de gestion des tournois
    ...
    manage_menu.add_option(4, "Initier un tournois", lambda: controller.start_tournament(tournament))
    manage_menu.add_option(5, "Saisir les résultats du round en cours", lambda: controller.record_current_round_results(tournament))
    manage_menu.add_option(0, "Retour", None)
    manage_menu.run()

#FIXME: Compléter la fonction
def display_report():
    reports_menu = Menu("Afficher les rapports")
    reports_menu.add_option(1, "Liste des joueurs de la FFE (ordre alphabétique)", lambda: print("⚠️ À implémenter"))
    reports_menu.add_option(2, "Liste de tous les tournois", lambda: print("⚠️ À implémenter"))
    reports_menu.add_option(3, "Liste des joueurs d'un tournois (ordre alphabétique)", lambda: print("⚠️ À implémenter"))
    reports_menu.add_option(4, "Liste des rounds et matchs d'un tournois", lambda: lambda: print("⚠️ À implémenter"))
