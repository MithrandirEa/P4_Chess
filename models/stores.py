...
def add_player(player: Player) -> bool:
    ...
    if any(
        (
            isinstance(rec, dict)
            and rec.get("national_chess_id") == player.national_chess_id
        )
        for rec in players
    ):
        return False
...
def create_round(
    name: str, number: Optional[int] = None, date: Optional[str] = None
) -> Round:
    data = _read_rounds_store()
...
def add_match_to_round(
    round_id: str, white_id: str, black_id: str, board: Optional[int] = None
) -> Optional[Match]:
