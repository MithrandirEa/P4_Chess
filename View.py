...
def show_match_added(m: 'Match') -> None:
    print(f"Match ajouté : {m.white_id} vs {m.black_id} (id={m.match_id}, table={m.board})")
...
def show_rounds(rounds: List['Round']) -> None:
    ...
    for r in rounds:
        ...
        if not r.matches:
            ...
            for m in r.matches:
                res = m.result or "—"
                print(f"    · [{m.match_id}] table={m.board}  {m.white_id} vs {m.black_id}  -> {res}")

def show_set_result(ok: bool) -> None:
    print("Résultat mis à jour." if ok else "Échec de la mise à jour (IDs/valeur invalides).")
